...
def inputfile():
    """"ファイル入力"""
    if len(sys.argv) != 4:
        print("使用法: python3 delaunay04.py <入力ファイル名> <建物の地面ファイル名> <出力ファイル名>")
        sys.exit(1)
...
def select_points(alldata):
    ...
    point_num = 0
    part_data = []
    select_points_dict = {(point[0], point[1]): point for point in alldata}
    missing_point_sum = 0
    for i,all_point in enumerate(alldata):
        #欠損点補完
        if all_point[3] == 0:
            print(i)
            neighbors_z = 0
            near_point_num = 0
            near_point_sum = 0
            x,y = all_point[0],all_point[1]
            #上3マス
            if (x-1,y+1) in select_points_dict:
                neighbors_z = select_points_dict[(x-1,y+1)][2] 
                if neighbors_z != -9999.99:
                    near_point_sum += neighbors_z
                    near_point_num += 1
            if (x,y+1) in select_points_dict:
                neighbors_z = select_points_dict[(x,y+1)][2] 
                if neighbors_z != -9999.99:
                    near_point_sum += neighbors_z
                    near_point_num += 1
            if (x+1,y+1) in select_points_dict:
                ...
                if neighbors_z != -9999.99:
                    ...
                    near_point_num += 1
            #左右2マス
            if (x-1,y) in select_points_dict:
                neighbors_z = select_points_dict[(x-1,y)][2] 
                if neighbors_z != -9999.99:
                    near_point_sum += neighbors_z
                    near_point_num += 1
            if (x+1,y) in select_points_dict:
            ...
            if near_point_num != 0:
                missing_point_sum += 1
                point_num += 1
                all_point[2] = near_point_sum / near_point_num
                print(f"欠損点補完を点{all_point}で行いました")
            else:
                print(f"{all_point}では周囲の8近傍が全て欠損点です")
            
            part_data.append(all_point)

        #属性値が1or2のものをappendする
        elif (all_point[3] == 1 or all_point[3] == 2) and all_point[2] <= 15:
            part_data.append(all_point)
            point_num += 1
    
    print(f"{int(missing_point_sum)}箇所で欠損点補完を行いました")
