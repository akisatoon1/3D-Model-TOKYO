...
#tool.0.0.1　ある近傍点の反対側の近傍点のidx確定 ver.2 GAN 25.10.23
def c4a(i, s):
    try:k=s.index(i)
...
#tool.0.1 符号付角度計算 ver.3 GAN 25.11.19
...
#step.2 基準値に最も近い未判定点idx確定 ver.3:GAN 25.10.23
def find_std(data):
    mindis=10000
    minidx = None
...
#tool.1 地面中心四近傍 ver.2 GAN 25.11.19
def ground_city(i):
    c = c4(i)
    for j in c:
...
def ground_plain(i):
    ...
    for j in c:
        ...
        if (dj >= 6) or (ang > 60) :data[j][3] = 3 #建物
        else:data[j][3] = 1 #地面

#tool.2 川中心四近傍 ver.2 GAN 25.11.19
def river(i): 
    c = c4(i)
    for j in c:
        if j is None:continue
...
#tool.3 建物中心四近傍 ver.2 GAN 25.11.19
def building_city(i):
    c=c4(i)
    for j in c:
        if j is None:continue
        ...
        if dj >= -6 or (ang > -75):data[j][3] = 3#建物
        else:data[j][3] = 1#地面
...
def building_plain(i):
    ...
    for j in c:
        ...
        if dj <= -4 or (ang < -35):data[j][3] = 1#地面
        else:data[j][3] = 3#建物

#tool.4 木中心四近傍 ver.1 GAN 25.10.19
def tree(i):
    c = c4(i)
    for j in c:
        if j is None:continue
...
#use xy to get index ver.3 GAN 25.11.19
idx_by_xy={}
for i in range(len(data)):
...
#step.1 基準値確定　ver.3:GAN 25.11.17
s = [0]*301
for i in data:
...
while any(row[3] is None for row in data):
    ...
    spread(groundD,   ground_plain,   1)
    spread(buildingD, building_plain, 3)
    spread(treeD,     tree,     4)
